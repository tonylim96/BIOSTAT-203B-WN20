...
The meaning of `"$1"`, `"$2"`, and `"$3"` in the shell script represents the order of the variables in the command `./middle.sh pride_and_prejudice.txt 20 5`. For example, in `head -n "$2" "$1"`, the `"$1"` refers to the `pride_and_prejudice.txt` and `"$2"` refers to 20, so that shows the first 20 lines in the `.txt.` file. That is piped into the `tail` command, and the `"$3"` refers to the 5, which means it only displays the last five lines. 
